...
function Sudeste() {
  ...
  const handleSimulation = async () => {
    try {
      const soilProperties = await fetchData("/soil_properties/sudeste");
      const initialConditions = await fetchData("/initial_conditions/sudeste");
      const boundaryConditions = await fetchData(
        "/boundary_conditions/sudeste"
      );
      const climateParameters = await fetchData("/climate_parameters/sudeste");
      const domainGeometry = await fetchData("/domain_geometry/sudeste");
  ...
  const handleDownload = () => {
    ...
    const data = `
    Dados para Simulação de bulbo umido na região Sudeste.
    ...
    a.href = url;
    a.download = "dados_para`_simulacao_de_bulbo_umido_no_Sudeste";
  ...
  return (
    <div className="container">
      <h2 className="header center-align">Simulação da Região Sudeste</h2>
      <div className="row">
